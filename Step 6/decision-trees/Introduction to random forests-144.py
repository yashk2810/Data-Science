...
auc = roc_auc_score(pre, test['high_income'])
auc2 = roc_auc_score(pre2, test['high_income'])
...
mean_pred = numpy.round((predictions + predictions2) / 2)
print(mean_pred)

auc = roc_auc_score(mean_pred, test['high_income'])
print(auc)
...
rounded_mean = numpy.round(sum(predictions) / len(predictions))
print(rounded_mean)
auc = roc_auc_score(rounded_mean, test['high_income'])
print(auc)
...
rfc = RandomForestClassifier(n_estimators=150, random_state=1, min_samples_leaf=75)
rfc.fit(train[columns], train['high_income'])

pred1 = rfc.predict(train[columns])
print(roc_auc_score(pred1, train['high_income']))

pred2 = rfc.predict(test[columns])
print(roc_auc_score(pred2, test['high_income']))