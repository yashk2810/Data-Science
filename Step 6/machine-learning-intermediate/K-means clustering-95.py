...
def assign_to_cluster(row):
    ...
    for i in centroids_dict:
        ed = calculate_distance(centroids_dict[i], [row['ppg'], row['atr']])
        euclid[i] = ed
    min_euclid_row = min(euclid, key=euclid.get) 
    return min_euclid_row
...
def recalculate_centroids(df):
    ...
    for cluster_id in range(0, num_clusters):
        ...
        avg_x = clustered['ppg'].mean()
        avg_y = clustered['atr'].mean() 
        
        new_centroids_dict[cluster_id] = [avg_x, avg_y]
