...
null_list = age_null.tolist()
age_null_count = null_list.count(True)
...
new_values=[]
age_null = pd.isnull(titanic_survival["age"])
#a = titanic_survival["age"]
#for i,j in zip(list(age_null),list(a)):
#    if i == False:
#        new_values.append(j)

#correct_mean_age = sum(new_values) / len(new_values)
...
for c in passenger_classes:
    specific_rows = titanic_survival.loc[titanic_survival["pclass"] == c]
    fare_column_mean = specific_rows["fare"].mean()
    fares_by_class[c] = fare_column_mean
...
row_1100_age = titanic_survival.loc[1100,'age']
row_25_survived = titanic_survival.loc[25,'survived']
...
n = titanic_survival.dropna(subset=['age','boat'])
titanic_reindexed = n.reset_index(drop=True)
...
def not_null_count(column):
    null_column = pd.isnull(column)
    not_null = null_column[null_column == False]
    return len(not_null)
...
age_labels = titanic_survival.apply(all_age, axis=1)
print (age_labels)
