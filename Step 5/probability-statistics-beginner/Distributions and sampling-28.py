...
lowest_income = income.loc[income['median_income'] == income['median_income'].min()]
lowest_income_county = ''.join(lowest_income['county'].values)
print(lowest_income_county)

m = income.loc[income['pop_over_25']>500000]
lowest = m[m['median_income'] == m['median_income'].min()]
lowest_income_high_pop_county = ''.join(lowest['county'].values)
...
random.seed(1)
print(random.sample(shopping, 3))
...
random.seed(1)
medium = [roll() for _ in range(100)]
plt.hist(medium, 6)
plt.show()

random.seed(1)
large = [roll() for _ in range(10000)]
plt.hist(large, 6)
plt.show()
...
random.seed(1)
medium = probability_of_one(300,100)
plt.hist(medium, 20)
plt.show()

random.seed(1)
large = probability_of_one(300, 1000)
plt.hist(large, 20)
plt.show()
...
mean_large = numpy.mean(large_sample)

diff = 0.18 - mean_large
deviations_from_mean = diff / large_sample_std
...
random.seed(1)
mean_random_rows = [select_random_sample(100)['median_income'].mean() for _ in range(1000)]
plt.hist(mean_random_rows, 20)
plt.show()
...
for _ in range(1000):
    ...
    mean_ratios.append((sample['median_income_hs'] / sample['median_income_college']).mean())

plt.hist(mean_ratios)
plt.show()
