...
d = [i-values_median for i in values]
median_difference_sum = sum(d)
...
for i in nba_stats['pts']:
    d = i-pts_mean
    v+=(d**2)
point_variance = v/len(nba_stats['pts'])
...
td1 = point_10-mean
point_10_std = td1 / std_dev

td2 = point_100 - mean
point_100_std = td2 / std_dev
...
r_fta_pts, p = pearsonr(nba_stats['fta'], nba_stats['pts'])
print(r_fta_pts)

r_stl_pf, p = pearsonr(nba_stats['stl'], nba_stats['pf'])
print(r_stl_pf)
...
def covariance(col1, col2):
    ...
    for i in range(len(col1)):
        mul = (col1[i]-col1_mean) * (col2[i]-col2_mean)
        s+=mul
    cov = s / len(col1)
    return cov
...
print(cov(nba_stats['fta'], nba_stats['blk'])[0,1])
